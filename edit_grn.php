...
if(isset($_POST['final_submit'])){
  ...
  if($db->query($query)){
   $session->msg('s',' GRN submitted!');
    ...
    ref_no,ref_source,product_id,stock_qty,stock_price,stock_by,stock_type,loc_id)


    (select   grn_id,'GRN', product_id,quantity,price,'{$submit_by}','receive','{$loc_id}'   from grn_details where grn_id='{$grn_id}' and quantity>0) ";
        ...
        <tbody>
          ...
          foreach ($all_requisition_details as $reqdetails): 
           ...
           <tr>
            ...
            <td><?php if($reqdetails['unit_type']=='number') echo intval($reqdetails['reqqty']); else echo  $reqdetails['reqqty']; ?></td>
            <td><?php if($reqdetails['unit_type']=='number') echo intval($reqdetails['received']); else echo  $reqdetails['received']; ?></td>



            <td>
              <input type="hidden" name="pidlist[]" value="<?php echo remove_junk($reqdetails['product_id']);?>"  >
              <input type="number" class="form-control" name="newqtylist[]"  
              value="<?php echo remove_junk($newqty);?>"     <?php if($reqdetails['unit_type']=='decimal')echo "step='any'"; ?>
               max="<?php echo remove_junk($pendqty);?>"          ></td>
              ...
              value="<?php echo remove_junk($reqdetails['price']);?>"     step="any"
              min="0"         ></td>
  ...
  <form method="post" action="edit_grn.php?id=<?php echo $grn_id;  ?>">
    <input  type="hidden" name="location"
     ...
     <input  type="hidden" name="requisition-id"
                 value="<?php echo  remove_junk($grn['req_id']); ?>" />             
