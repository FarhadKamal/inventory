...
$all_trans_details = find_by_sql('select transfer_details.*,products.id as pid, products.name,
  ...
  if($db->query($query)){
   ...
   ifnull(quantity*(select round(sum(stock.stock_price)/sum(stock.stock_qty),2) from stock where stock.product_id=transfer_details.product_id),0) as price,'{$submit_by}','receive','{$loc_to}'   
  ...
  // echo  $query;
   ...
   ifnull(-1*quantity*(select round(sum(stock.stock_price)/sum(stock.stock_qty),2) from stock where stock.product_id=transfer_details.product_id),0) as price,'{$submit_by}','issue','{$loc_from}'   
   from transfer_details where tran_id='{$track_id}' and quantity>0) ";
