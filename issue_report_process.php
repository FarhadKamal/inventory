...
if(isset($_POST['submit'])){
  ...
  if(empty($errors)){
    ...
    $cost         = remove_junk($db->escape($_POST['cost-centre-name']));
    ...
    $results      = find_issue_by_dates($start_date,$end_date,$location,$product,$cost  );
...
<head>
 ...
 <title>Issue History Report</title>
       ...
       table thead tr th {
         ...
         border: 1px solid #ededed;
           ...
           <body>
            ...
            onclick="tableToExcel('excel', 'Issue History Report', 'IssueReport.xls')" 
               ...
               <table class="table table-border">
                ...
                <tbody>
                  ...
                  foreach($results as $result):
                   ...
                   $total_price= ($result['price']*$result['quantity'])+ $total_price;

                   ?>
                   <tr>
                    <td class="text-right"><?php echo count_id() ;?></td>

                    <td ><?php echo ($result['con_date']);?></td>

                    <td ><?php echo ($result['cost_name']);?></td>
                    <td ><?php echo ($result['loc_name']);?></td>
                    <td ><?php echo ($result['consumer']);?></td>
                   ...
                   <?php }  ?>
                   <td align="right"><?php echo sprintf('%0.2f',$result['price']*$result['quantity']);?></td>
        ...
        $session->msg("d", "Sorry no issue has been found. ");
        redirect('issue_report.php', false);
      endif;
      ?>
    </body>
