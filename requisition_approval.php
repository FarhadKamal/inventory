    ...
    <table class="table table-striped table-bordered" id="example" style="width:100%"> 
      ...
      <tbody>
        <?php foreach ($requisition as $req):
          ...
          ?>
          <tr >
            <td <?php  if($req['approve_status']==0 and $req['cancel_status']==0 )echo 'style="background-color: #FFCCCB;"';?>  ><?php echo count_id();?></td>

            <td class="text-center"> <?php echo remove_junk($req['id']); ?></td>
            <td> <?php echo remove_junk($req['claimer']); ?></td>
            <td> <?php echo remove_junk($req['submit_date']); ?></td>
            ...
            <td <?php  if($req['approve_status']==0 and $req['cancel_status']==0 )echo 'style="background-color: #FFCCCB;"';

              else if($req['approve_status']==1 and $req['cancel_status']==0 and $req['complete_status']==1 )echo 'style="background-color: #00FF00;"'; 
            ...
            <td class="text-center">
              <div>
                 ...
                 <?php
                if($req['approve_status']==0 and $req['cancel_status']==0 )
                { 
                  ?>
                  <a href="edit_requisition.php?id=<?php echo (int)$req['id'];?>" class="btn btn-info btn-xs"  title="Process" data-toggle="tooltip">
                ...
                { 
                  ?>
                  <a href="cancel_requisition_approval.php?id=<?php echo (int)$req['id'];?>" class="btn btn-danger btn-xs"  title="Delete" onclick="return confirm('Are you sure you want to cancel? Requisition ID:<?php echo (int)$req["id"];?> ');" data-toggle="tooltip">
