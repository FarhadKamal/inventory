	...
	function generate_short_code($id)
	{
		...
		if($total==0)
		{
			...
			if(strlen($digit)==1)$zero=".000";
			else if(strlen($digit)==2)$zero=".00";
			else if(strlen($digit)==3)$zero=".0";
	...
	function generate_short_code_edit($id,$oid,$pid)
	{
		...
		if($id<>$oid)$newcode=generate_short_code($id);
		else
		{
			global $db;
	...
	function join_grn_table(){
		...
		$sql  .=" where cancel_status=0 ORDER BY g.id desc";
		return find_by_sql($sql);
	...
	function join_issue_table(){
		...
		$sql  .=" where cancel_status=0 ORDER BY c.id desc";
		return find_by_sql($sql);
	...
	function join_issue_return_table(){
		...
		$sql  .=" inner join locations l on l.id=c.loc_id";
		$sql  .=" where cr.cancel_status=0  ORDER BY cr.id desc";
		return find_by_sql($sql);
	...
	function join_requisition_table(){
		global $db;
		$sql  =" SELECT r. *, u.name as claimer, apv.name as holder";

		$sql  .=" FROM requisition r";
		$sql  .=" inner join users u on u.id=r.submit_by";
		$sql  .=" left join users apv on apv.id=r.hold_by";
		$sql  .=" where cancel_status=0 ORDER BY r.id desc";
		return find_by_sql($sql);
	...
	function join_requisition_proc_table(){
		global $db;
		$sql  =" SELECT r. *, u.name as claimer, apv.name as holder,GROUP_CONCAT(distinct rd.assigned SEPARATOR '----') as asgn";

		$sql  .=" FROM requisition r";
		$sql  .=" inner join users u on u.id=r.submit_by";
		$sql  .=" inner join requisition_details rd on rd.req_id=r.id";
		$sql  .=" left join users apv on apv.id=r.hold_by";
		$sql  .=" where cancel_status=0 and approve_status=1 Group BY r.id desc";
		return find_by_sql($sql);
	...
	function join_adjustment_table(){
		...
		$sql  .=" where cancel_status=0 ORDER BY a.id desc";
		return find_by_sql($sql);
...
/*--------------------------------------------------------------*/
	...
	function get_requisition_by_id($id){
		global $db;
		$sql  =" SELECT r. *, u.name as claimer, apv.name as holder,u.designation as udes";
	...
	/*--------------------------------------------------------------*/
	function find_stock_by_dates($start_date,$end_date,$loc){
	global $db;
	...
	/*--------------------------------------------------------------*/
	function find_issue_by_dates($start_date,$end_date,$loc,$pid,$cid){
	global $db;
	$start_date  = date("Y-m-d", strtotime($start_date));
	$end_date    = date("Y-m-d", strtotime($end_date));
    ...
    $sql .= " inner join locations l on l.id=c.loc_id  ";
	$sql .= " where cancel_status=0 and submit_status=1 and ";
	...
	/*--------------------------------------------------------------*/
	function find_purchase_by_dates($start_date,$end_date,$loc,$pid,$sid){
	global $db;
	$start_date  = date("Y-m-d", strtotime($start_date));
	$end_date    = date("Y-m-d", strtotime($end_date));
	...
	/* Function for Generate stock report by two dates
	/*--------------------------------------------------------------*/
	function find_stock_ledger_by_dates($start_date,$end_date,$loc,$pid){
	global $db;
	$start_date  = date("Y-m-d", strtotime($start_date));
	$end_date    = date("Y-m-d", strtotime($end_date));
	...
	/* Function for Generate stock report by two dates
	/*--------------------------------------------------------------*/
	function find_opening_stock_ledger_by_dates($start_date,$end_date,$loc,$pid){
	...
	/*--------------------------------------------------------------*/
	function product_tracking_by_dates($start_date,$end_date,$loc,$pid){
	global $db;
	$start_date  = date("Y-m-d", strtotime($start_date));
	$end_date    = date("Y-m-d", strtotime($end_date));
	...
	/*--------------------------------------------------------------*/
	function track_product_stock($start_date,$end_date,$loc,$pid){
