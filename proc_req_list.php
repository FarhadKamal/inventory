...
$requisition = join_requisition_proc_table();
?>
      ...
      <tbody>
        <?php foreach ($requisition as $req):
          ...
          <tr>
            ...
            <td> <?php echo remove_junk($req['asgn']); ?></td>
            ...
            $recom_array= explode(",",$req['recom_track']);

            if(!in_array( $_SESSION['user_id'], $recom_array)     ){ ?>

                <a href="recom_requisition_approval.php?id=<?php echo (int)$req['id'];?>" class="btn btn-info btn-xs"  title="Recommend" onclick="return confirm('Are you sure you want to Recommend? Requisition ID:<?php echo (int)$req["id"];?> ');" data-toggle="tooltip">
