    ...
    $all_returns = find_by_sql('select * from consume where  submit_status=1 and cancel_status=0 order by id desc limit 100');
    ...
    if(isset($_POST['add_issue_return'])){
      ...
      if(empty($errors)){
        ...
        $query .=" con_id,rtn_by,rtn_date,remarks";
        $query .=") VALUES (";
        $query .=" '{$con_id}', '{$submit_by}', now(), '{$remarks}'";
        $query .=")";
