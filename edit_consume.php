...
$all_cost = find_by_sql('select * from  cost_centre order by cost_name');
...
$all_issue_details = find_by_sql('select consume_details.*,products.id as pid, products.name,
  ...
  if($db->query($query)){
   ...
   -1*quantity*(select round(sum(stock.stock_price)/sum(stock.stock_qty),2) from stock where stock.product_id=consume_details.product_id),'{$submit_by}','issue','{$loc_id}'   
