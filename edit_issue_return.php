...
$all_issue_details = find_by_sql('select consume_return_details.*,products.id as pid, products.name,
  ...
  if($db->query($query)){
   ...
   ifnull(quantity*(select round(sum(stock.stock_price)/sum(stock.stock_qty),2) from stock where stock.product_id=consume_return_details.product_id),0),'{$submit_by}','receive','{$loc_id}'   
